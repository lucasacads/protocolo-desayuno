  ...
  return (
    <button
      ...
      className={`
        ...
        flex flex-col items-center justify-center py-4 px-3 sm:px-6
    ...
    >
      ...
      <div className="flex items-center justify-center gap-2 text-lg sm:text-xl md:text-2xl font-black uppercase tracking-wide z-10 text-shadow-sm w-full">
        <ShoppingCart className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 fill-white/20 flex-shrink-0" />
        <span className="drop-shadow-md text-center leading-tight">{text}</span>
        <ArrowRight className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 flex-shrink-0" />
      </div>
      {subtext && (
        <span className="text-xs sm:text-sm font-bold text-yellow-300 opacity-100 mt-1 z-10 uppercase tracking-wider drop-shadow-sm text-center">
          {subtext}
        </span>
      )}
